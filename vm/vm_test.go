...
func testExpectedObject(t *testing.T, expected interface{}, actual object.Object) {
	...
	case *object.Error:
		errObject, ok := actual.(*object.Error)
		if !ok {
			t.Errorf("object is not Error. got=%T (%+v)", actual, actual)
			return
		}

		if errObject.Message != expected.Message {
			t.Errorf("wrong error message. expected=%q, got=%q",
				expected.Message, errObject.Message)
		}
	}
}
...
func TestFunctionsWithReturnStatements(t *testing.T) {
	tests := []vmTestCase{
		{
			input: `
		let earlyExit = cl() { return 99; 100; };
		earlyExit();
		`,
			expected: 99,
		},
		{
			input: `
		let earlyExit = cl() { return 99; return 100; };
		earlyExit();
			`,
			expected: 99,
		},
	...
	tests := []vmTestCase{
		{
			input: `
		let returnsOne = cl() { 1; };
		let returnsOneReturner = cl() { returnsOne; };
		returnsOneReturner()();
		`,
			expected: 1,
		},
...
func TestCallingFunctionsWithBindings(t *testing.T) {
	tests := []vmTestCase{
		...
		{
			input: `
		let oneAndTwo = cl() { let one = 1; let two = 2; one + two; };
		oneAndTwo();
		`,
			expected: 3,
		},
		{
			input: `
		let oneAndTwo = cl() { let one = 1; let two = 2; one + two; };
		let threeAndFour = cl() { let three = 3; let four = 4; three + four; };
		...
		{
			input: `
		let firstFoobar = cl() { let foobar = 50; foobar; };
		let secondFoobar = cl() { let foobar = 100; foobar; };
...
func TestCallingFunctionsWithWrongNumberOfArguments(t *testing.T) {
	...
	for _, tt := range tests {
		...
		if err.Error() != tt.expected {
			t.Fatalf("wrong error. want=%q, got=%q", tt.expected, err)
		}
	}
}
