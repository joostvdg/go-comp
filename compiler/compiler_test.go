...
func testConstants(t *testing.T, expected []interface{}, actual []object.Object) error {
	...
	for i, constant := range expected {
		...
		case []code.Instructions:
			...
			err := testInstructions(constant, function.Instructions)
			if err != nil {
...
func testInstructions(expected []code.Instructions, actual code.Instructions) error {
	...
	if len(actual) != len(concatted) {
		return fmt.Errorf("wrong instructions length.\nwant=%q\ngot_=%q", concatted, actual)
	}

	for i, ins := range concatted {
		if actual[i] != ins {
			return fmt.Errorf("wrong instruction at %d.\nwant=%q\ngot_=%q", i, concatted, actual)
		}
	}
...
func TestCompilerScopes(t *testing.T) {
	...
	if len(compiler.scopes[compiler.scopeIndex].instructions) != 1 {
		t.Errorf("instruction length wrong. got=%d, want=%d", len(compiler.scopes[compiler.scopeIndex].instructions), 1)
	}
