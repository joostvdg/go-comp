...
func TestResolveFree(t *testing.T) {
	...
	for _, tt := range tests {
		...
		if len(tt.table.FreeSymbols) != len(tt.expectedFreeSymbols) {
			t.Errorf("wrong number of free symbols. want=%d, got=%d",
				len(tt.expectedFreeSymbols), len(tt.table.FreeSymbols))
			continue
		}

		for i, symbol := range tt.expectedFreeSymbols {
			result := tt.table.FreeSymbols[i]
			if result != symbol {
				t.Errorf("expected free symbol %v, got %v", symbol, result)
			}
		}
	}
}
...
func TestResolveUnresolvableFree(t *testing.T) {
	...
	for _, name := range expectedUnresolvable {
		...
		if ok {
			t.Errorf("name %s should not be resolvable", name)
		}
	}
